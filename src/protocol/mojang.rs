...
impl Profile {
    pub fn login(username: &str, password: &str, token: &str) -> Result<Profile, super::Error> {
        ...
        let req = r#try!(serde_json::to_string(&req_msg));
        ...
        let ret: serde_json::Value = r#try!(serde_json::from_reader(res));
    ...
    pub fn refresh(self, token: &str) -> Result<Profile, super::Error> {
        ...
        if res.status() != reqwest::StatusCode::NO_CONTENT {
            let req = r#try!(serde_json::to_string(&req_msg)); // TODO: fix parsing twice to avoid move
            // Refresh needed
