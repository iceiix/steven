...
impl Serializable for Statistic {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Self, Error> {
        Ok(Statistic {
            name: r#try!(Serializable::read_from(buf)),
            value: r#try!(Serializable::read_from(buf)),
        })
    }

    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(self.name.write_to(buf));
...
impl Serializable for BlockChangeRecord {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Self, Error> {
        Ok(BlockChangeRecord {
            xz: r#try!(Serializable::read_from(buf)),
            y: r#try!(Serializable::read_from(buf)),
            block_id: r#try!(Serializable::read_from(buf)),
        })
    }

    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(self.xz.write_to(buf));
        r#try!(self.y.write_to(buf));
...
impl Serializable for ExplosionRecord {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Self, Error> {
        Ok(ExplosionRecord {
            x: r#try!(Serializable::read_from(buf)),
            y: r#try!(Serializable::read_from(buf)),
            z: r#try!(Serializable::read_from(buf)),
        })
    }

    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(self.x.write_to(buf));
        r#try!(self.y.write_to(buf));
        self.z.write_to(buf)
    }
}
...
impl Serializable for MapIcon {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Self, Error> {
        Ok(MapIcon {
            direction_type: r#try!(Serializable::read_from(buf)),
            x: r#try!(Serializable::read_from(buf)),
            z: r#try!(Serializable::read_from(buf)),
        })
    }
    ...
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(self.direction_type.write_to(buf));
        r#try!(self.x.write_to(buf));
        self.z.write_to(buf)
    }
}
...
impl Serializable for EntityProperty {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Self, Error> {
        Ok(EntityProperty {
            key: r#try!(Serializable::read_from(buf)),
            value: r#try!(Serializable::read_from(buf)),
            modifiers: r#try!(Serializable::read_from(buf)),
        })
    }

    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(self.key.write_to(buf));
        r#try!(self.value.write_to(buf));
...
impl Serializable for PropertyModifier {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Self, Error> {
        Ok(PropertyModifier {
            uuid: r#try!(Serializable::read_from(buf)),
            amount: r#try!(Serializable::read_from(buf)),
            operation: r#try!(Serializable::read_from(buf)),
        })
    }

    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(self.uuid.write_to(buf));
        r#try!(self.amount.write_to(buf));
...
impl Serializable for PlayerInfoData {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Self, Error> {
        let mut m = PlayerInfoData {
            action: r#try!(Serializable::read_from(buf)),
            players: Vec::new(),
        };
        let len = r#try!(VarInt::read_from(buf));
        for _ in 0..len.0 {
            let uuid = r#try!(UUID::read_from(buf));
            match m.action.0 {
                0 => {
                    let name = r#try!(String::read_from(buf));
                    let mut props = Vec::new();
                    let plen = r#try!(VarInt::read_from(buf)).0;
                    for _ in 0..plen {
                        let mut prop = PlayerProperty {
                            name: r#try!(String::read_from(buf)),
                            value: r#try!(String::read_from(buf)),
                            signature: Default::default(),
                        };
                        if r#try!(bool::read_from(buf)) {
                            prop.signature = Some(r#try!(String::read_from(buf)));
                        }
                        props.push(prop);
                    }
                    let p = PlayerDetail::Add {
                        ...
                        name: name,
                        properties: props,
                        gamemode: r#try!(Serializable::read_from(buf)),
                        ping: r#try!(Serializable::read_from(buf)),
                        display: {
                            if r#try!(bool::read_from(buf)) {
                                Some(r#try!(Serializable::read_from(buf)))
                            } else {
                                None
                            }
                        },
