                                ...
                                Direction::$dirName => {
                                    ...
                                    $(
                                        self::$state::$dir::internal_ids::$name => {
                                            ...
                                            $(
                                                if true $(&& ($cond(&packet)))* {
                                                    packet.$field = r#try!(Serializable::read_from(&mut buf));
                                                }
                                            )+
...
impl Serializable for Option<nbt::NamedTag>{
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Option<nbt::NamedTag>, Error> {
        let ty = r#try!(buf.read_u8());
        if ty == 0 {
            Result::Ok(None)
        } else {
            let name = r#try!(nbt::read_string(buf));
            let tag = r#try!(nbt::Tag::read_from(buf));
    ...
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        match *self {
            Some(ref val) => {
                r#try!(buf.write_u8(10));
                r#try!(nbt::write_string(buf, &val.0));
                r#try!(val.1.write_to(buf));
            }
            None => r#try!(buf.write_u8(0)),
        }
        Result::Ok(())
    }
}
...
impl <T> Serializable for Option<T> where T : Serializable {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Option<T>, Error> {
        Result::Ok(Some(r#try!(T::read_from(buf))))
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        if self.is_some() {
            r#try!(self.as_ref().unwrap().write_to(buf));
        }
        Result::Ok(())
    }
}
...
impl Serializable for String {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<String, Error> {
        ...
        r#try!(buf.take(len as u64).read_to_string(&mut ret));
        Result::Ok(ret)
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        let bytes = self.as_bytes();
        r#try!(VarInt(bytes.len() as i32).write_to(buf));
        r#try!(buf.write_all(bytes));
        Result::Ok(())
    }
}
...
impl Serializable for format::Component {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Self, Error> {
        let len = r#try!(VarInt::read_from(buf)).0;
        let mut ret = String::new();
        r#try!(buf.take(len as u64).read_to_string(&mut ret));
...
impl Serializable for i8 {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<i8, Error> {
        Result::Ok(r#try!(buf.read_i8()))
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(buf.write_i8(*self));
        Result::Ok(())
    }
}
...
impl Serializable for i16 {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<i16, Error> {
        Result::Ok(r#try!(buf.read_i16::<BigEndian>()))
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(buf.write_i16::<BigEndian>(*self));
        Result::Ok(())
    }
}
...
impl Serializable for i32 {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<i32, Error> {
        Result::Ok(r#try!(buf.read_i32::<BigEndian>()))
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(buf.write_i32::<BigEndian>(*self));
        Result::Ok(())
    }
}
...
impl Serializable for i64 {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<i64, Error> {
        Result::Ok(r#try!(buf.read_i64::<BigEndian>()))
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(buf.write_i64::<BigEndian>(*self));
        Result::Ok(())
    }
}
...
impl Serializable for u8 {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<u8, Error> {
        Result::Ok(r#try!(buf.read_u8()))
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(buf.write_u8(*self));
        Result::Ok(())
    }
}
...
impl Serializable for u16 {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<u16, Error> {
        Result::Ok(r#try!(buf.read_u16::<BigEndian>()))
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(buf.write_u16::<BigEndian>(*self));
        Result::Ok(())
    }
}
...
impl Serializable for u64 {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<u64, Error> {
        Result::Ok(r#try!(buf.read_u64::<BigEndian>()))
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(buf.write_u64::<BigEndian>(*self));
        Result::Ok(())
    }
}
...
impl Serializable for f32 {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<f32, Error> {
        Result::Ok(r#try!(buf.read_f32::<BigEndian>()))
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(buf.write_f32::<BigEndian>(*self));
        Result::Ok(())
    }
}
...
impl Serializable for f64 {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<f64, Error> {
        Result::Ok(r#try!(buf.read_f64::<BigEndian>()))
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(buf.write_f64::<BigEndian>(*self));
        Result::Ok(())
    }
}
...
impl Serializable for UUID {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<UUID, Error> {
        Result::Ok(UUID(r#try!(buf.read_u64::<BigEndian>()),
                        r#try!(buf.read_u64::<BigEndian>())))
    }
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        r#try!(buf.write_u64::<BigEndian>(self.0));
        r#try!(buf.write_u64::<BigEndian>(self.1));
        Result::Ok(())
    }
}
...
impl <L: Lengthable, V: Serializable>  Serializable for LenPrefixed<L, V> {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<LenPrefixed<L, V>, Error> {
        let len_data: L = r#try!(Serializable::read_from(buf));
        let len: usize = len_data.into();
        let mut data: Vec<V> = Vec::with_capacity(len);
        for _ in 0..len {
            data.push(r#try!(Serializable::read_from(buf)));
        }
    ...
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        ...
        r#try!(len_data.write_to(buf));
        let data = &self.data;
        for val in data {
            r#try!(val.write_to(buf));
        }
        Result::Ok(())
    }
}
...
impl <L: Lengthable>  Serializable for LenPrefixedBytes<L> {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<LenPrefixedBytes<L>, Error> {
        ...
        r#try!(buf.take(len as u64).read_to_end(&mut data));
...
impl Serializable for VarInt {
    ...
    fn read_from<R: io::Read>(buf: &mut R) -> Result<VarInt, Error> {
        ...
        let mut val = 0u32;
        loop {
            let b = r#try!(buf.read_u8()) as u32;
            val |= (b & PART) << (size * 7);
            size += 1;
            if size > 5 {
    ...
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        ...
        loop {
            if (val & !PART) == 0 {
                r#try!(buf.write_u8(val as u8));
                return Result::Ok(());
            }
            r#try!(buf.write_u8(((val & PART) | 0x80) as u8));
            val >>= 7;
        }
    }
}
...
impl Serializable for VarLong {
    ...
    fn read_from<R: io::Read>(buf: &mut R) -> Result<VarLong, Error> {
        ...
        let mut val = 0u64;
        loop {
            let b = r#try!(buf.read_u8()) as u64;
            val |= (b & PART) << (size * 7);
            size += 1;
            if size > 10 {
...
impl Serializable for Position {
    fn read_from<R: io::Read>(buf: &mut R) -> Result<Position, Error> {
        let pos = r#try!(buf.read_u64::<BigEndian>());
        Ok(Position::new(
    ...
    fn write_to<W: io::Write>(&self, buf: &mut W) -> Result<(), Error> {
        ...
        r#try!(buf.write_u64::<BigEndian>(pos));
        Result::Ok(())
    }
}
...
impl Conn {
    pub fn new(target: &str) -> Result<Conn, Error> {
        ...
        let stream = r#try!(TcpStream::connect(&*address));
        Result::Ok(Conn {
            stream: stream,
    ...
    pub fn write_packet<T: PacketType>(&mut self, packet: T) -> Result<(), Error> {
        let mut buf = Vec::new();
        r#try!(VarInt(packet.packet_id()).write_to(&mut buf));
        r#try!(packet.write(&mut buf));
        ...
        if self.compression_threshold >= 0 && buf.len() as i32 > self.compression_threshold {
            ...
            let mut new = Vec::new();
            r#try!(VarInt(uncompressed_size as i32).write_to(&mut new));
            let write = self.compression_write.as_mut().unwrap();
            write.reset(io::Cursor::new(buf));
            r#try!(write.read_to_end(&mut new));
            buf = new;
        }

        r#try!(VarInt(buf.len() as i32 + extra).write_to(self));
        if self.compression_threshold >= 0 && extra == 1 {
            r#try!(VarInt(0).write_to(self));
        }
        r#try!(self.write_all(&buf));

        Result::Ok(())
    }

    pub fn read_packet(&mut self) -> Result<packet::Packet, Error> {
        let len = r#try!(VarInt::read_from(self)).0 as usize;
        let mut ibuf = vec![0; len];
        r#try!(self.read_exact(&mut ibuf));
        ...
        let packet = r#try!(packet::packet_by_id(self.state, dir, id, &mut buf));
    ...
    pub fn do_status(mut self) -> Result<(Status, Duration), Error> {
        ...
        r#try!(self.write_packet(StatusRequest { empty: () }));

        let status = if let Packet::StatusResponse(res) = r#try!(self.read_packet()) {
            res.status
        } else {
        ...
        r#try!(self.write_packet(StatusPing { ping: 42 }));

        if let Packet::StatusPong(_) = r#try!(self.read_packet()) {
        } else {
            return Err(Error::Err("Wrong packet".to_owned()));
        };
        ...
        let version = r#try!(val.get("version").ok_or(invalid_status()));
        let players = r#try!(val.get("players").ok_or(invalid_status()));

        Ok((Status {
            version: StatusVersion {
                name: r#try!(version.get("name").and_then(Value::as_str).ok_or(invalid_status()))
                          .to_owned(),
...
impl Read for Conn {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        match self.cipher.as_mut() {
            ...
            Option::Some(cipher) => {
                let ret = r#try!(self.stream.read(buf));
    ...
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        match self.cipher.as_mut() {
            ...
            Option::Some(cipher) => {
                ...
                r#try!(self.stream.write_all(&data));
                Ok(buf.len())
            }
        }
    }
